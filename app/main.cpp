 ...
 * po przesunieciu atomu relaksowana jest lokalnie sieć w celu zmniejszenia
 *napręzeń
 *
 * atomy początkowo są rozmieszczone w węzłach siatki, w wyniku naprężeń mogą
 *się nieco przesunąć poza węzeł
 **********************************************************************************************************/
...
int main() {
  ...
  std::size_t irange_min{10};  // w zakresie (xi,yj)+/- irange liczne sa lokalne
                               // zmiany polozen atomow
  std::size_t irange_max{
      10};  // maksymalny rozmiar otoczenia w relaksacji lokalnej
  ...
  int idiffusion{2};  // 1-dyfuzja atomow podloza i deponowanych, 2-dyfuzja
                      // tylko atomow deponowanych (podloze stabilne)

  double time_fluence{8.0};
  double time_simulation{100.0};

  long unsigned iterations{1000};  // maksymalna liczba iteracji w trakcie
                                   // rozwiazywania ukladu rownan
  double tolerance{1.0E-3};        // tolerancja bledu w iteracyjnej metodzie
                                   // rozwiazywania ukaldu rownan

  int n_write_files{1000};  // zapis danych do pliku co tyle iteracji
  int n_global_relaxation{
      1000};  // wykonanie relaksacji globalnej co tyle iteracji

  // symulacja
  growth_simulation(as, ag, al, skl, skd, mu, D, E, gamma, nx, ny, nsurf,
                    irange_min, irange_max, tol_local_max, time_fluence,
                    time_simulation, k_max_step, f_deposition, temperature,
                    idiffusion, iterations, tolerance, n_write_files,
                    n_global_relaxation);
}  // main
