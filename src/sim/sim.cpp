...
void growth_simulation(double as, double ag, double al, double skl, double skd, double mu, double D, double E, double gamma,
				std::size_t nx, std::size_t ny, std::size_t nsurf, std::size_t irange_min, std::size_t irange_max, double tol_local_max, 
			     double time_fluence, double time_simulation, std::size_t k_max_step,
			     double f_deposition, double temperature, int idiffusion, long unsigned iterations, const double tolerance,
...
*/		
  ...
  double kbt {k_boltzmann*temperature/ev};  //energia termiczna w [eV]
    ...
    {		
		...
		//********* INFO ************************************************	
			...
			printf("iter, time, new_atoms, pr_acc, (real time [s]), duv_max =    %6d    %12.6f  %6d  %12.4f   (%12.1f)    %10.3f\n", 
				 iter,time,number_atoms,n_accepted/n_proposed,static_cast<double>(czas.count())*1.0E-3, duv_max);
	 ...
	 *********************************************************************************************************************/
      ...
      fluence=0.;
		rdep=(static_cast<double>(k_max_step)+1.0)*(2*static_cast<double>(k_max_step)+1.0)*fluence/6.; 
	 ...
	 **********************************************************************************************************************/	
		...
		for(std::size_t i=0;i<nx;i++){
			for(std::size_t j=ny-1;j>=1;j--){
				long int ll=lround(crystal[i][j][0]); //ll=0(brak), 1(podloze), 2(atom wiazki)
				if(ll>0){					
					if(ll>=idiffusion){
						n_at_diff++;
						atoms_diff[n_at_diff][0]=static_cast<double>(lround(crystal[i][j][0])); //typ atomu
						...
						for(int ii=-1;ii<=1;ii++){
							for(int jj=-1;jj<=1;jj++){
								if((abs(ii)+abs(jj))>0){
									std::size_t i2= static_cast<std::size_t>((static_cast<int>(i)+ii+static_cast<int>(nx))%static_cast<int>(nx));
									std::size_t j2 = static_cast<std::size_t>(j + static_cast<std::size_t>(jj));
						...
						r0=12.*D/(static_cast<double>(k_max_step)+1.0)/(2*static_cast<double>(k_max_step)+2.);
	...
	// losujemy proces: depozycja lub dyfuzja atomu	
		...
		std::size_t ktory=n_at_diff; //zabezpieczenie na wypadek gdyby petla nie zadziala
		for(std::size_t i=0;i<=n_at_diff;i++){
		 ...
		 ***************************************************************************************************/ 
			...
			std::size_t ipos=gen_discrete_1_K(nx)-1; //polozenie atomu w kierunku x: 0-(nx-1)
			std::size_t jpos=0;
			for(std::size_t j=ny-1;j>=1;j--){
		 ...
		 ****************************************************************************************************/
			...
			std::size_t ii_shift=gen_discrete_1_K_multiply_sign(k_max_step); //losowe przesuniecie lewo-prawo
			std::size_t i_old=static_cast<std::size_t>(lround(atoms_diff[ktory][1])); //aktualna pozycja atomu dyfundujacego
			std::size_t j_old=static_cast<std::size_t>(lround(atoms_diff[ktory][2])); //aktualna pozycja
		...
		//**** liczymy stara energie [-irange,irange]:  atom-on  *****************************
			...
			for(int i=-irange;i<=irange; i++){
				for(int j=-irange;j<=irange; j++){
					std::size_t ii=static_cast<std::size_t>((static_cast<int>(i_old)+i+static_cast<int>(nx))%static_cast<int>(nx));
					std::size_t jj=static_cast<std::size_t>(static_cast<int>(j_old)+j);
		...
		//**** liczymy nowa energie - atom-off [-range,irange]*****************************
			...
			for(int i=-irange;i<=irange; i++){
				for(int j=-irange;j<=irange; j++){
					int ii=static_cast<int>((i_old+static_cast<std::size_t>(i)+nx)%nx);
					int jj=static_cast<int>(j_old+static_cast<std::size_t>(j));
					if(jj>1 && static_cast<std::size_t>(jj)<(ny-1) && crystal_copy[static_cast<std::size_t>(ii)][static_cast<std::size_t>(jj)][0]>0.5){
						en_new+=compute_elastic_energy_wij(static_cast<std::size_t>(ii),static_cast<std::size_t>(jj),crystal_copy,as,ag,al,skl,skd); //elastic energy
					}
				}
			}
		...
		//******* sprawdzamy czy prawdopodobienstwo r_new<r_old=r_approx - jesli tak to atom dyfunduje   ***********************
				double neigh_d=std::round(atoms_diff[ktory][7]); //aktualna liczba najblizszych sasiadow
				r0=12.*D/(static_cast<double>(k_max_step)+1.0)/(2*static_cast<double>(k_max_step)+2.);
				double ri_new=r0*exp((-gamma*neigh_d+(en_old-en_new)/2+E)/kbt); //dokladne tempo dyfuzji atomu
