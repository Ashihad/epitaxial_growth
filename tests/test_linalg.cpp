...
TEST(LinalgTest, LargeSparseMatrixTest) {
    ...
    double *csr_val = static_cast<double *>(malloc(sizeof(double) * (n * 2)));  // Overestimate number of non-zeros
    int *csr_row = static_cast<int *>(malloc(sizeof(int) * (n + 1)));
    int *csr_column = static_cast<int *>(malloc(sizeof(int) * (n * 2)));  // Overestimate number of non-zeros

    // Input vector x
    double *x = static_cast<double *>(malloc(sizeof(double) * n));
    double *y = static_cast<double *>(malloc(sizeof(double) * n));  // Output vector y
    ...
    // Expected output: since x is all 1's, y should reflect the sum of non-zero elements in each row
